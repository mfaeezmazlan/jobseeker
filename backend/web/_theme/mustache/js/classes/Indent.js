...
function hasBlockElements(elements) {
    for (var e in elements)
        if (elements.hasOwnProperty(e)) {
            if (elements[e].name in blockTags)
                return true;
        }
    return false;
}
...
function pretty_print(dom, level) {
    ...
    for (var i = 0; i < dom.length; i++) {
        ...
        if (e.type == 'tag' || e.type == 'script' || e.type == 'style') {
            ...
            if ("attribs" in e) {
                for (var name in e.attribs)
                    if (e.attribs.hasOwnProperty(name)) {
                        var attribWrap = '"';
            ...
            if (e.name in emptyTags) {
                ...
                output += ">";
                if (pWithBlocks)
                    output += "</" + e.name + ">";//close the p tag here cause that's enough (only good for ACE!)
                ...
                if (e.type == 'script' || e.type == 'style') {
                    ...
                    if (i < dom.length - 1 && (dom[i + 1].type == 'comment'))
                        ;//don't add new line if adjacent next is a comment, we add new line after comment
            ...
            if (text.length > 0) {
                if (e.type == 'comment' && i > 0 && dom[i - 1].type == 'tag') {
                    ...
                    if (e.type == 'text' && e.data.match(/^\S/) && i > 0 && dom[i - 1].type == 'tag')
                        ;//don't add tabs if text doesn't start with "SPACE"
                    else
                        for (var t = 0; t < level; t++)
                            output += "\t";
