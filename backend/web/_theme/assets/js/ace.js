    ...
    $$.getAttrSettings = function (el, attr_list, prefix) {
        ...
        for (var li in attr_list)
            if (attr_list.hasOwnProperty(li)) {
                ...
                if (!((attr_val = el.getAttribute(prefix + attr_name))))
                    continue;
...
(function ($, undefined) {
    ...
    function AceAjax(contentArea, settings) {
                            ...
                            if (link_element.length > 0) {
                                ...
                                if (nav.length > 0) {
                                    ...
                                    nav.find('.active').each(function () {
                                        ...
                                        if ($(this).hasClass('hover') || self.settings.close_active)
                                            $class += ' open';
        ...
        this.startLoading = function () {
            ...
            if (this.settings.loading_overlay == 'body')
                $('body').append($overlay.addClass('ajax-overlay-body'));

            else if (this.settings.loading_overlay)
                $(this.settings.loading_overlay).append($overlay);
        ...
        if (!hash && this.settings.default_url)
    ...
    $.fn.aceAjax = $.fn.ace_ajax = function (option, value, value2, value3) {
        ...
        var $set = this.each(function () {
            ...
            if (typeof option === 'string' && typeof data[option] === 'function') {
                ...
                if (value3 != undefined)
                    method_call = data[option](value, value2, value3);
    ...
    function Sidebar(sidebar, settings) {
        ...
        this.$sidebar.on(ace.click_event + '.ace.submenu', '.nav-list', function (ev) {
            ...
            if (!link_element || link_element.length == 0)
                return;//return if not clicked inside a link element
            ...
            if (sub_hidden) {//being shown now
                ...
                if (height_change != 0)
                    height_change += sub.scrollHeight;//we need new updated 'scrollHeight' here
        ...
        var sidebar_mobile_style = function () {
            ...
            else if (this.$sidebar.hasClass('navbar-collapse'))
                this.mobile_style = 4;//collapsible (bootstrap style)
    ...
    var is_element_pos =
            ...
            function Sidebar_Scroll(sidebar, settings) {
                ...
                var initiate = function (on_page_load) {
                    ...
                    if (!self.settings.include_toggle)
                        $toggle.css({'z-index': 1});

                    if (!self.settings.include_shortcuts)
                    ...
                    if (self.settings.scroll_outside == true)
                ...
                this.prehide = function (height_change) {
                    ...
                    if (!this.is_scrolling || ace_sidebar.get('minimized'))
                        return;//when minimized submenu's toggle should have no effect
...
(function ($, undefined) {
    ...
    var is_element_pos =
            ...
            function Sidebar_Hover(sidebar, settings) {
                ...
                if (hasTouch)
                    self.settings.sub_hover_delay = parseInt(Math.max(self.settings.sub_hover_delay, 2500));//for touch device, delay is at least 2.5sec
                ...
                $sidebar.on(event_1, '.nav-list li, .sidebar-shortcuts', function (e) {
                    ...
                    if (sidebar_vars['collapsible'] /**|| sidebar_vars['horizontal']*/)
                        return;
                ...
                function adjust_submenu(sub) {
                    ...
                    if (horizontal) {
                        ...
                        if ($li.parent().parent().hasClass('hover-flip'))
                            $li.addClass('hover-flip');//if a parent is already flipped, flip it then!
...
(function ($, undefined) {
    ...
    var switch_direction = function () {
        ...
        function applyChanges(el) {
            ...
            if (!el)
                $body.toggleClass('rtl');//el is 'body'
            ...
            if ($.fn.ace_scroll)
                $container.find('.scroll-hz').ace_scroll('reset') //to reset scrollLeft
